...
SELECT
  bu.*,
  (SELECT row_to_json(rc) FROM role_cte rc) AS ""Role"",
  (SELECT COALESCE(array_agg(pc.code), ARRAY[]::text[]) FROM perm_cte pc) AS ""PermissionCodes"",
  (SELECT row_to_json(v) FROM vendor_cte v) AS ""Vendor"",
  (SELECT row_to_json(k) FROM kyc_cte k) AS ""Kyc"",
  (SELECT COALESCE(json_agg(a), '[]'::json) FROM addr_cte a) AS ""Addresses"",
  (SELECT row_to_json(p) FROM pref_cte p) AS ""Preferences"",
  (SELECT row_to_json(n) FROM notif_cte n) AS ""Notifications"",
  (SELECT COALESCE(json_agg(s), '[]'::json) FROM sess_cte s) AS ""Sessions""
FROM base_user bu;
";
...
SELECT a.id AS ""Id"", a.title AS ""Title"", a.country AS ""Country"", a.city AS ""City"",
       a.district AS ""District"", a.postal_code AS ""PostalCode"", a.address_line AS ""AddressLine"",
       a.is_default AS ""IsDefault"", a.status AS ""Status""
FROM sys.user_addresses a
WHERE a.user_id = @userId AND a.status <> 'DELETED'
";

        public const string GetSelfSessions = @"
SELECT s.id AS ""Id"", s.device AS ""Device"", s.user_agent AS ""UserAgent"", s.ip_address AS ""IpAddress"",
       s.created_at AS ""CreatedAt"", s.last_seen_at AS ""LastSeenAt"", s.is_current AS ""IsCurrent""
FROM sys.user_sessions s
WHERE s.user_id = @userId
";

        public const string RevokeSelfSession = @"
DELETE FROM sys.user_sessions
